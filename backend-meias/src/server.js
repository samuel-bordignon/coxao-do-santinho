...
app.post('/movimentacoes', async (req, res) => {
  const { material_id, usuario_id, tipo, quantidade, data_movimentacao } = req.body || {};
  ...
  try {
    ...
    const ins = await client.query(
      `INSERT INTO movimentacoes (material_id, usuario_id, tipo, quantidade, data_movimentacao)
       VALUES ($1,$2,$3,$4,COALESCE($5, NOW()))
       RETURNING id, material_id, usuario_id, tipo, quantidade, data_movimentacao`,
      [material_id, usuario_id, String(tipo).toLowerCase(), Math.abs(quantidade), data_movimentacao || null]
    );
...
// histórico geral ou filtrado por produto (?material_id=)
app.get('/movimentacoes', async (req, res) => {
  const { material_id } = req.query;
  const hasFilter = !!material_id;
  const sql = `
    SELECT m.id, m.material_id, p.nome AS produto_nome,
           m.usuario_id, u.nome AS responsavel_nome,
           m.tipo, m.quantidade, m.data_movimentacao, 
      FROM movimentacoes m
      JOIN materiais p ON p.id = m.material_id
